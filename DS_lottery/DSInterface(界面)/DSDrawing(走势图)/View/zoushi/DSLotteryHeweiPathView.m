    ...
    for (NSInteger j = self.rowCount; j < self.rowCount + 4; j++){
        for (int i = 0; i < self.sectionArray.count; i++) {
            ...
            if(j - self.rowCount == 0) {
                ...
                [[NSString stringWithFormat:@"%ld",self.rowCount/[[dic objectForKey:@"all"]integerValue]] drawInRect:CGRectMake(i*self.width/self.listArray.count, j*cellHeight + top,self.width/self.listArray.count,cellHeight)withAttributes:dict];
