...
-(id)initWithFrame:(CGRect)frame  title:(NSString *)title content:(NSMutableArray *)contentArray{
    ...
    if (self) {
        ...
        UILabel *ti = [[UILabel alloc]initWithFrame:CGRectMake(0, 0,frame.size.width , frame.size.height/2)];
        ti.text = title;
        ...
        for (int i = 0; i < contentArray.count; i++) {
            ...
            luckLabel *num=[[luckLabel alloc]initWithFrame:CGRectMake(i * (frame.size.width/contentArray.count), CGRectGetMaxY(ti.frame), (frame.size.width/contentArray.count), frame.size.height/2)];
